...
def testa(largura, comprimento, historico):
    cartografia = { 90: "O",  180 : "N" , 270 : "L",  0: "S"} #Quando pegar o modulo da posição por 360 irá dar a orientação do robô
    posicao = 180 #Começa na posição Norte (tomei 180° como norte e 360° como sul)
    coordX = 0 
    coordY = 0
    for i in range(len(historico)):
        if historico[i]=='D': #Como é no sentido horário, quando vira para direita soma 90
            posicao += 90
        if historico[i] == 'E': #Como é no sentido anti-horário, quando vira para direita diminui 90
            posicao -= 90
